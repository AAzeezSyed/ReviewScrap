...
# base url + /
# http://localhost:8000 + /
@app.route('/review', methods = ['POST'])  # route with allowed methods as POST and GET
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            del bigboxes[0:3]  # the first 3 members of the list do not contain relevant information,
            # hence deleting them.
            box = bigboxes[0]  # taking the first iteration (for demo)
            productLink = "https://www.flipkart.com" + box.div.div.div.a['href']  # extracting the actual product link
            prodRes = requests.get(productLink)  # getting the product page from server
            prod_html = bs(prodRes.text, "html.parser")  # parsing the product page as HTML
            prod_Detail = prod_html.find("div", {"class": "CXW8mj _3nMexc"})
            prod_Details = prod_html.find("div", {"class": "_1AuMiq P9aMAP"})
            # ProductImage = bigboxes[0].div.img['src']
            ProductImage = prod_Details.div['style']
            ProductName = prod_Detail.img['alt']
            ...
            return render_template('results.html', reviews=reviews, productname = ProductName, productimage = ProductImage)  # showing the review to the user
        except Exception as e:
            return  render_template('results.html'), f'something is wrong {e}'
    else:
        return render_template('index.html')
